...
Soru çeşitliliği: Bir ders turu üç uygulama sorusu ve beş soruluk mini testten oluşur. Sorular turun tohumuyla karılan bir desteden gelir; aynı turda bir soru, hatta aynı yönerge ikinci kez çıkmaz. Öğren ekranındaki keşif sorusu da turda yeniden sorulmaz. “Yeniden keşfet” yeni bir tohumla başlar, sorular ve sıraları değişir. Hayat Bilgisi derslerinde 12’şer durum, İngilizce derslerinde 8’er kelime vardır. Matematik konuları birkaç soru türünü karıştırır, örneğin sayı doğrusu, sıralama, karşılaştırma ve günlük problem. Sanat, müzik ve hareket derslerinde üç uygulama görevi birbirinden farklıdır; düşünme soruları daha geniş bir havuzdan seçilir. Türkçe hikâye derslerinde her hikâyenin sekiz sorusu vardır; tekrar açılınca sıra ve yanlış seçenekler değişir. `tests/variety.test.ts` bu kuralları her ders için yüzlerce turda denetler.
...
Arayüz sıcak krem bir zemin, yumuşak katmanlı gölgeler, yuvarlak kartlar ve basıldığında çöken “tombul” düğmelerle kuruludur. Başlıklar Fredoka, metinler Nunito ile yazılır. Maskot Piko (el sallayan, sevinen, düşünen, kitap okuyan ve baş), yedi öğrenme adası, dört oyun, dört çiçek, yedi rozet ile ana sayfa ve başarı bahçesi sahneleri Codex `imagegen` ile aynı stil yönergesiyle üretilmiş 3B kil görsellerdir. Dosyalar `public/art/*.webp` altındadır ve `components/Illustration.tsx` içindeki `Art` bileşeniyle kullanılır; metni tamamlayan görseller `alt=""` ile ekran okuyucudan gizlenir. Görsellerde insan yoktur; Lara’yı temsil eden bir çizim bulunmaz. Soru kutularındaki sahneler (Hayat Bilgisi durumları, matematikte cisimler ve günlük nesneler) ile sayma simgeleri de aynı stilde kil görsellerdir (`public/art/hb`, `q`, `i`; ayrıntılar [Görseller](docs/ART.md)). Etkinlik araçları da kil görsellerle kurulur: trafik sahnesi (sokak, araba, yaya ışığı, yürüyen Piko), yön haritası, onluk bloklar, kesirdeki pizza/pasta/turta, madeni paralar ve kumbara, saat çerçevesi, cetveldeki kalem, geometrik cisimler, hareket ve konuşma görevlerinde görevi canlandıran Piko, ritimde alkışlayan Piko, örüntü simgeleri ve İngilizce kelime resimleri (`public/art/tool`, `en`, `i`, `piko-*`). Doğruluk gerektiren parçalar görselin üstüne HTML/SVG olarak çizilir: para değerleri, saat rakamları ve çizgileri, yaya ışığındaki duran/yürüyen simgesi, harita yolu. Düzlemsel şekiller köşe işaretleri tam oturduğu için SVG kalır. İngilizcede insan ve vücut kelimeleri (anne, öğretmen, göz gibi) görsel kuralı gereği emojidir.

Adlandırma kuralı: ders adası `island-<ders id>`, oyun `game-<oyun id>`, rozet `badge-*`, çiçek `flower-*`. Yeni ders veya oyun eklenirse aynı adla bir görsel de eklenmelidir. Ana ekran ikonları `deploy/app-icon.png` kaynağından, sekme ikonu `public/icon.svg`’den üretilir. Adaların ve Piko pozlarının yarım boyutlu `-sm` kopyaları da vardır; tarayıcı ekran yoğunluğuna göre seçer. Bütün görseller Service Worker önbelleğine girer (toplam yaklaşık 3,1 MB). Hareketler (sayfa geçişi, Piko’nun süzülmesi, ders sonu konfetisi) azaltılmış hareket tercihinde kapanır; sayfa geçişi saydamlık kullanmaz, böylece metin karşıtlığı hiçbir karede düşmez.
...
Unit testler: yanıt, puan, tekrar, uyarlama, tamamlama, yedek doğrulama, soru üretici sınırları, bir ders turunda soru tekrarı olmaması, Türkçe sayı ekleri, oyun mantığı ve müfredat ilişkileri. E2E: dört oyunun baştan sona oynanışı, tam matematik akışı, kalıcı ilerleme, ebeveyn kilidi/yedek, ders rotaları, offline açılış ve 360×800, 390×844, 768×1024, 1024×768, 1280×720, 1920×1080 ekranlarında Axe taraması. Lighthouse raporları `artifacts/` altında oluşturulur. Tarayıcı testleri gerçek cihaz ergonomisi veya öğretmen değerlendirmesinin yerine geçmez.
