...
// Statik yayında (node'suz) gerçekçi PNG override'larının yüklenmesi için gerekir.
...
function tara() {
  ...
  // Dolgun (balık etli) beden varyantları: _dolgun/<slot>/<id>.png
  const dolgun = {};
  for (const slot of GORSEL_SLOTLAR) {
...
if (require.main === module) {
  ...
  const say = Object.entries(veri).reduce((t, [k, m]) =>
    t + (k === "dolgun" || k === "beden" ? 0 : Object.keys(m).length), 0);
