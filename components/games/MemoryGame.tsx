...
export default function MemoryGame({sound,onFinish}:{sound:boolean;onFinish:(moves:number)=>void}){
 ...
 function flip(i:number){
  ...
  const next=[...open,i];setOpen(next);if(next.length<2)return;
  const count=moves+1,[a,b]=next.map(j=>cards[j]);setMoves(count);
  if(a.pair===b.pair){const all=[...found,a.pair];setFound(all);setOpen([]);setMessage('Eşini buldun! 🎉');tone(sound,784);if(all.length===pairs){onFinish(count);timer.current=setTimeout(()=>setFinished(true),900)}}
  else{setMessage('Bu iki kart eş değil. Yerlerini aklında tut!');tone(sound,262,.1);timer.current=setTimeout(()=>setOpen([]),1100)}
 }
