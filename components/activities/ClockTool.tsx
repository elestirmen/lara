...
export default function ClockTool({q,onAnswer,disabled,reveal=false,explore=false}:{q:Question;onAnswer:(s:string)=>void;disabled:boolean;reveal?:boolean;explore?:boolean}){
 ...
 const change=(n:number)=>{const next=wrapMinutes(n);setTotal(next);onAnswer(clockAnswer(next))};
 ...
 return <div className="clock-lab">
  ...
  <div className="clock-adjust"><label>Saat<select aria-label="Saat" value={hour} disabled={disabled} onChange={e=>change(Number(e.target.value)*60+minute)}>{Array.from({length:12},(_,i)=><option key={i} value={i+1}>{i+1}</option>)}</select></label><label>Dakika<select aria-label="Dakika" value={minute} disabled={disabled} onChange={e=>change(hour*60+Number(e.target.value))}>{[...new Set([0,15,30,45,minute])].sort((a,b)=>a-b).map(i=><option key={i} value={i}>{String(i).padStart(2,'0')}</option>)}</select></label>{!explore&&<button disabled={disabled} onClick={()=>onAnswer(clockAnswer(total))}>Bu saati seç</button>}</div>
