"use strict";
/*
  Yönetim (admin) paneli — gardıroba kolayca gerçekçi görsel ekleme/silme.
  Şifre ile korunur (sunucudaki LARA_ADMIN, varsayılan "lara2018").
  Yükleme node sunucusu gerektirir (statik yayında /api yoktur) → yerel adreste kullan.
*/
(function () {
  ...
  const ETIKET = {
    ...
    asalar: "✨ Asa", arkaplanlar: "🌅 Arka plan",
  };
  ...
  async function api(yol, govde) {
    ...
    if (!r.ok && r.status !== 401 && r.status !== 400) {
      throw new Error("yok"); // 404 vb → backend yok (statik yayın)
    }
    return r.json();
  }
  ...
  async function giris() {
    ...
    try {
      ...
      hata.textContent = "Yükleme yalnızca yerel sunucuda çalışır (örn. http://<bilgisayar-ip>:8080). perinet.org statik yayındır.";
      hata.classList.remove("gizli");
    }
  }
  ...
  async function dosyaKontrol() {
    ...
    const tip = f.type === "image/png" ? "png" : (f.type === "image/webp" ? "webp" : null);
    const satir = [];
    const ekle = (ok, txt) => satir.push(`<div class="${ok ? 'iyi' : 'kotu'}">${ok ? '✓' : '✗'} ${txt}</div>`);

    if (!tip) { ekle(false, "Dosya PNG (veya WebP) olmalı"); kutu.innerHTML = satir.join(""); return; }
    ...
    const oranTamam = Math.abs(w / h - 3 / 4) < 0.02;
    ekle(oranTamam, `Boyut: ${w}×${h} ${oranTamam ? "(3:4 ✔)" : "→ 3:4 olmalı (1024×1365)"}`);
    if (oranTamam && (w !== 1024 || h !== 1365)) ekle(true, "Not: ideal 1024×1365 (yine de uyumlu)");

    const slot = $("#yonSlot").value;
    if (slot === "arkaplanlar") {
      ekle(true, "Arka plan: tam ekranı doldurmalı (opak olabilir)");
    } else {
      const saydam = saydamlikVar(img);
      ekle(saydam, saydam ? "Arka plan şeffaf" : "Arka plan şeffaf DEĞİL gibi (parça dışı şeffaf olmalı)");
    }
    ...
    // Yükleme için zorunlu: tür + oran (şeffaflık uyarı amaçlı, engellemez)
    if (tip && oranTamam) { seciliDosya = { dataUrl, tip }; btn.disabled = false; }
  }
