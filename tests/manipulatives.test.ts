...
describe('somut öğrenme araçları',()=>{
 ...
 it('simetri deseni aynaya olan uzaklığı korur',()=>{const l=lessons.find(l=>l.id==='math-symmetry')!;for(let seed=0;seed<40;seed++)for(let i=0;i<8;i++){const q=makeQuestion(l,i,1,seed);if(i===0)expect(q.type).toBe('geometry');if(q.type!=='geometry')continue;expect(q.items!.map(Number).map(n=>Math.floor(n/3)*3+2-n%3).sort((a,b)=>a-b).join('|')).toBe(q.answer)}});
